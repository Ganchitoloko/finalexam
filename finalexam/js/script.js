...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function addListAfterClick() {
    ...
    if (inputLength() > 0 && !taskExists(task)) {
      // makes sure that an empty input field doesn't create a li and checks for duplicates
  ...
  function addListAfterKeypress(event) {
    let task = input.value.trim();
    if (inputLength() > 0 && event.which === 13 && !taskExists(task)) {
      // this now looks to see if you hit "enter"/"return" and checks for duplicates
